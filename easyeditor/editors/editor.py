...
class BaseEditor:
    ...
    def __init__(self, hparams: HyperParams):
        ...
        if type(self.model_name) is str:
            ...
            if "t5" in self.model_name.lower():
                ...
                self.model = AutoModelForCausalLM.from_pretrained(
                    self.model_name, torch_dtype=torch_dtype, trust_remote_code=True, device_map=device_map
                )
                self.tok = AutoTokenizer.from_pretrained(self.model_name, trust_remote_code=True)
                self.tok.pad_token_id = self.tok.eos_token_id
            elif "chatglm" in self.model_name.lower():
                self.model = AutoModel.from_pretrained(
                    self.model_name, trust_remote_code=True, torch_dtype=torch_dtype, device_map=device_map
                )
                self.tok = AutoTokenizer.from_pretrained(self.model_name, trust_remote_code=True)
                ...
                self.model = AutoModelForCausalLM.from_pretrained(
                    self.model_name, fp32=False, trust_remote_code=True, device_map=device_map
                )
                self.tok = AutoTokenizer.from_pretrained(
                    self.model_name,
                    eos_token="<|endoftext|>",
            ...
            if (
                ...
                and (hparams.alg_name not in ["ROME", "MEMIT", "EMMET", "R-ROME"])
            ):
            ...
            if (
                ...
                and (hparams.alg_name in ["ROME", "MEMIT", "EMMET", "R-ROME"])
            ):
        ...
        **kwargs,
        ...
        if "requests" in kwargs.keys():
            ...
            requests = _prepare_requests(
                prompts, target_new, ground_truth, rephrase_prompts, locality_inputs, portability_inputs, **kwargs
            )
    ...
    def batch_edit(
        ...
        **kwargs,
        ...
        requests = _prepare_requests(
            prompts, target_new, ground_truth, rephrase_prompts, locality_prompts, locality_ground_truth, **kwargs
        )
