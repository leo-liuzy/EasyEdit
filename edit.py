...
def test_KE():
    prompts = [
        "Who is the architect for Toodyay Fire Station?",
        "Who is Claire Clairmont's sister?",
    ...
    metrics, edited_model, _ = editor.batch_edit(
        prompts=prompts + prompts, ground_truth=ground_truth + ground_truth, target_new=target_new + target_new
    )
...
def test_ROME():
    ...
    editor = BaseEditor.from_hparams(hparams)
    metrics, edited_model, _ = editor.edit(
        prompts=prompts, ground_truth=ground_truth, target_new=target_new, subject=subject, keep_original_weight=True
    )
...
def test_MEND():
    ...
    editor = BaseEditor.from_hparams(hparams)
    metrics, edited_model, _ = editor.edit(
        prompts=prompts, ground_truth=ground_truth, target_new=target_new, sequential_edit=False
    )
...
def test_KE():
    ...
    editor = BaseEditor.from_hparams(hparams)
    metrics, edited_model, _ = editor.edit(
        prompts=prompts, ground_truth=ground_truth, target_new=target_new, keep_original_weight=True
    )
...
def test_SERAC():
    ...
    ground_truth = ["BBC", "politician", "Antarctica", "Malaysia", "Istanbul"]
    target_new = ["Sega", "actor", "Europe", "India", "Naples"]
    import json
...
def test_IKE():
    ...
    editor = BaseEditor.from_hparams(hparams)
    metrics, edited_model, _ = editor.edit(
        prompts=prompts, ground_truth=ground_truth, target_new=target_new, train_ds=train_ds, keep_original_weight=True
    )
...
def test_ROME_DEMO():
    ...
    editor = BaseEditor.from_hparams(hparams)
    metrics, edited_model, _ = editor.edit(
        prompts=prompts, target_new=target_new, subject=subject, keep_original_weight=False, verbose=False
    )
    ...
    pre_edit_outputs = model.generate(
        input_ids=batch["input_ids"].to("cuda"), attention_mask=batch["attention_mask"].to("cuda"), max_new_tokens=3
    )
    ...
    print(
        "Pre-Edit Outputs: ",
        [tokenizer.decode(x, skip_special_tokens=True) for x in pre_edit_outputs.detach().cpu().numpy().tolist()][0],
    )
    print(
        "\033[1;35mPost-Edit\033[0m Outputs: ",
        [tokenizer.decode(x, skip_special_tokens=True) for x in post_edit_outputs.detach().cpu().numpy().tolist()][
            0
        ].replace("Boris Johnson", "\033[1;35mBoris Johnson\033[0m"),
    )  # exit()
...
def ROME_DEMO_2():
    ...
    post_edit_outputs = edited_model.generate(
        input_ids=batch["input_ids"].to("cuda"), attention_mask=batch["attention_mask"].to("cuda"), max_new_tokens=8
    )
...
def test_melo():
    ...
    editor = BaseEditor.from_hparams(hparams)
    metrics, edited_model, _ = editor.edit(
        prompts=prompts, ground_truth=ground_truth, target_new=target_new, subject=subject, keep_original_weight=False
    )
